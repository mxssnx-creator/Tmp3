// Force rebuild: 2026-05-05T16:00:00 — Systemic fix for "deleted main connections being auto-re-added" + "Reset DB key count stays high":
//   1. lib/redis-migrations.ts → ensureBaseConnections() now STRICTLY preserves all operator flag choices (is_active_inserted, is_inserted, is_dashboard_inserted, is_enabled, is_enabled_dashboard, is_active) for existing connections. Only credential rotation from env touches the row. autoActive defaults apply ONLY on first-time seed (brand-new DB / tombstone-recovered connection). Fixes: every cold-start was unconditionally setting is_active_inserted="1" for Bybit/BingX, undoing the operator's DELETE on /api/settings/connections/[id]/active.
//   2. app/api/admin/clear-progressions/route.ts → switched from include-list of 28 patterns to allowlist scan (KEYS * → keep PROTECTED prefixes only, delete everything else). Adds startingKeyCount + endingKeyCount + deleteErrors to the response so silent-failure cases ("delete returned 0") become visible in the operator toast. Also adds connection:test:* and connection:rate_limit:* to a force-clear list so they're wiped despite sharing the connection: prefix.
