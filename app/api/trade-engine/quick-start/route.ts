...
function isMainAssigned(connection: any): boolean {
  // Prefer explicit main-assignment flag with backward-compatible fallbacks.
  return connection?.is_assigned === "1" ||
    connection?.is_assigned === true ||
    connection?.is_active_inserted === "1" ||
    connection?.is_active_inserted === true ||
    connection?.is_dashboard_inserted === "1" ||
    connection?.is_dashboard_inserted === true
}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    let connection = allConnections.find((c: any) => {
      ...
      const isAssigned = isMainAssigned(c)
      ...
      return NextResponse.json(
        { 
          ...
          availableConnections: allConnections.map((c: any) => ({ 
            ...
            isMainAssigned: isMainAssigned(c),
          })),
    ...
    console.log(`${LOG_PREFIX}: [3/4] Stored symbols in trade_engine_state: ${symbols.join(", ")}`)
     ...
     const isAssigned = isMainAssigned(updated)
