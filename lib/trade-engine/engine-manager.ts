  ...
  private startIndicationProcessor(intervalSeconds: number = 1): void {
    ...
    this.indicationTimer = setInterval(async () => {
      ...
      try {
        ...
        // so the parallel processIndication calls all hit the in-memory cache (zero extra round-trips).
        await prefetchMarketDataBatch(symbols).catch(() => { /* non-critical */ })

        // Process indications for every symbol in parallel.
        // Uses module-level caches (SHARED_MARKET_DATA_CACHE) so there are no `this` context
        // issues even in stale webpack bundles.
    ...
    this.strategyTimer = setInterval(async () => {
      ...
      try {
        ...
        try {
          await ProgressionStateManager.incrementCycle(this.connectionId, true, 0)
          if (cycleCount % 10 === 0) {
            console.log(`[v0] [StrategyCycles] ${this.connectionId}: cycleCount=${cycleCount}, evaluated=${evaluatedThisCycle}, liveReady=${liveReadyThisCycle}`)
          }
        } catch (incError) {
