  ...
  private startIndicationProcessor(intervalSeconds: number = 1): void {
    ...
    this.indicationTimer = setInterval(async () => {
      ...
      try {
        ...
        // CRITICAL: Completely bypass the broken IndicationProcessor class
        // The class method fails because this.marketDataCache is undefined in cached webpack bundles
        // Indications are now generated by the /api/cron/generate-indications endpoint via client hook
        // This loop just returns empty arrays to prevent errors - actual indications come from the cron job
        const indicationResults = await Promise.all(
          symbols.map(async (symbol) => {
            // Return empty - indications are generated by /api/cron/generate-indications
